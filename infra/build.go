...
func main() {
	...
	for _, themeDir := range themeDirs {
		themeDirF, err := os.Open(fmt.Sprint(themesDir, "/", themeDir.Name()))
		must(err)
		...
		for _, themeFile := range themeFiles {
			if themeFile.IsDir() && themeFile.Name() != "gtk-4.0" {
				dirToDel := fmt.Sprint(themesDir, "/", themeDir.Name(), "/", themeFile.Name())
